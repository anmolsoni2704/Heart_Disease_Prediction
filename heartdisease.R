...
#import the data
heart <- read.csv("C:/Users/win10/Desktop/heartDataset.csv")
View(heart)
...
#Boxplot 1
heart1=data.frame(heart$age, heart$trestbps,heart$chol,heart$thalach)
...
boxplot(age~cp, data = heart, xlab = "No. of Age", ylab = "Gender", main = "Details of Heart Attack", col=c("green","red"))
...
table(heart$target)
#152/303
#The baseline model value of 0.5, 
#means that approximately 50% of patients suffering from heart disease.
...
heartlog=glm(target ~ target+age+trestbps+chol+fbs+restecg+thalach+exang+oldpeak+slope+ca+thal,data=qualityTrain,family = binomial)
summary(heartlog)
...
heartlog2=glm(target ~ age+sex+cp+trestbps+chol+fbs+thalach+exang+oldpeak+slope+ca+thal,data=qualityTrain,family = binomial)
summary(heartlog2)
...
#AUC value is 0.895 means the model is able to distinguish 
#between patients with the disease and no disease with a 
#probability of 0.895. So it is a good value.
...
#attributes performed best with an accuracy of testing 81%.